    ...
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add_parent' && $user_role == 'admin') {
            ...
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO users (username, full_name, password, phone, role, status) VALUES (?, ?, ?, ?, 'parent', 'active')");
            ...
            try {
                ...
                $stmt = $pdo->prepare("UPDATE users SET full_name = ?, phone = ?, status = ? WHERE id = ? AND role = 'parent'");
                $stmt->execute([$full_name, $phone ?: null, $status, $parent_id]);
